...
ages = []
races=[]
for file in files:#이미지에서 나이, 인종을 판별하는 문자열 저장
...
model = Model(inputs=inputs, outputs=[age_model, race_model]) 

#model compile
model.compile(optimizer = 'adam', loss =['mse','mse'],metrics=['mae','accuracy', precision, recall])
...
#model 저장
model.save('UTKface_age_race_model.h5')
...
hist_csv_file = 'age_race_history.csv'
