...
genders = []
races=[]
for file in files:#이미지에서 나이, 성별, 인종을 판별하는 문자열 저장
...
target = np.zeros((size, 3),dtype='float32')
features = np.zeros((size, sample.shape[0], sample.shape[1], 1),dtype = 'float32')
for i in range(size):
    target[i,0] = age_group(int(ages[i])) / 4
    target[i,1] = int(genders[i])
    target[i,2] = int(races[i])
    features[i] = images[i]
...
#model compile
model.compile(optimizer = 'adam', loss =['mse','binary_crossentropy','mse'],metrics=['mae','accuracy', precision, recall])

model.summary()

h = model.fit(x_train,[y_train[:,0],y_train[:,1],y_train[:,2]], validation_data=(x_test,[y_test[:,0],y_test[:,1],y_test[:,2]]), epochs = 25, callbacks=[early_stopping], batch_size=128, shuffle = True)
...
#model 측정
score= model.evaluate(x_test,[y_test[:,0],y_test[:,1],y_test[:,2]])
...
hist_csv_file = 'age_gender_race_history.csv'
